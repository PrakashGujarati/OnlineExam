          ...
          @if ($answers)
            @foreach ($filtStudents as $key => $student)
              <tr>
                ...
                <td>
                @foreach ($uniqueuseranswer as $ua)
                 @if ($student->id == $ua->user_id)
                 {{$ua->created_at}}
                 @endif
                 @endforeach
                </td>
                <td>
                  <a data-toggle="modal" data-target="#delete{{ $topic->id }}" title="It will delete the answer sheet of this student" href="#" class="btn btn-sm btn-danger">
                    Reset Exam
                  </a>
