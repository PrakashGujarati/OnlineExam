...
class QuizList extends Component{
      ...
      quizListLoad(){
        axios.get('http://localhost:8000/api/questionList')
        .then(response => {
          const quizlist = response.data;
          
          let quiz = JSON.parse(JSON.stringify(this.props.quiz));
          quiz.quizlist = Object.assign(this.props.quiz.quizlist|| {}, quizlist);
                  ...
                  <div key={list.id} className="col-sm-4">
                    <div className="card">
                      <div className="card-body">
                        ...
                        <p className="card-text">Total Question : {list.totalquestion}</p>
                        <p className="card-text">Time : {list.totalquestion}</p>
                        <button className="btn btn-primary" onClick={() => this.quizSelectHandler(list.id)} >Start Quiz</button>
                      </div>
                    </div>
                  </div>
              );
...
const mapDispatchToProps = dispatch => ({
  onQuizListLoad: payload => dispatch({ type: ActionTypes.QuizListLoad, payload }),
