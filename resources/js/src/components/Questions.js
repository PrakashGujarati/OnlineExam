...
class Questions extends Component {
    onAnswer(question, option) {
        ...
        if (option.id === 1) user_answer = "A";
        if (option.id === 2) user_answer = "B";
        if (option.id === 3) user_answer = "C";
        if (option.id === 4) user_answer = "D";
        ...
        axios.post(`${API_URL}/questionList`, data).then(response => {
    ...
    clearResponse() {
        ...
        let q =
            quiz.questions[
                parseInt(this.props.match.params.id) || this.props.pager.index
            ];
        q.options.forEach(x => {
            x.selected = false;
        });
        ...
        axios.post(`${API_URL}/api/clearResponse`, data).then(response => {
            // console.log(response);
        });
...
const mapDispatchToProps = dispatch => ({
    ...
    onClearResponse: payload =>
        dispatch({ type: ActionTypes.ClearResponse, payload })
});
