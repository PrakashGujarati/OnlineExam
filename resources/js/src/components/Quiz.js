...
class Quiz extends Component {
    ...
    load() {
        ...
        let url = `${API_URL}/questionList/` + topic_id;
        fetch(url)
            .then(res => res.json())
            .then(res => {
                let quiz = res;
                quiz.config = Object.assign(
    ...
    move = e => {
        ...
        this.props.history.replace(
            "/quiz/" + this.props.match.params.topic_id + "/" + index
        );
        switch (id) {
            case "first":
                index = 0;
    ...
    renderMode() {
        const renderer = ({ hours, minutes, seconds, completed }) => {
            if (completed) {
                ...
                // });
                axios
                    .post(`${API_URL}/updateCountdown`, data)
                    .then(response => {
                        console.log(response);
                    });
                return (
        ...
        if (this.props.quiz.status === "quiz") {
            if (
                this.props.quiz.hours === "0" &&
                this.props.quiz.minutes === "0" &&
                this.props.quiz.seconds === "0"
            ) {
                timer = (
                    <Countdown
                        date={
                            Date.now() +
                            parseInt(this.props.quiz.timer) * 60 * 1000
                        }
                        //date={Date.now() + 10000}
                        renderer={renderer}
                    />
            ...
            return (
                <div className="row">
                    <div className="col-12">{timer}</div>
                    <div className="col-8">
                    ...
                    <div className="col-4">
                        <div className="card bg-light">
                            <div className="card-body">
                                <h4 className="text-center font-weight-small">
                                    Review Quiz
                                </h4>
                                <hr />
                                <Review
    ...
    render() {
        return <div id="quiz">{this.renderMode()}</div>;
    }
}

const mapStateToProps = state => {
    return { ...state.quiz, ...state.pager };
};

const mapDispatchToProps = dispatch => ({
    onQuizLoad: payload => dispatch({ type: ActionTypes.QuizLoad, payload }),
    onQuizResultLoad: payload =>
        dispatch({ type: ActionTypes.QuizResultLoad, payload }),
